...
async def main() -> None:
    st.set_page_config(
        page_title="Celeste Domain Template", page_icon="🚀", layout="wide"
    )
    ...
    with st.sidebar:
        ...
        if providers:
            ...
            st.error(
                "No models found for this capability. "
                "Make sure providers are configured."
            )
            st.stop()
        ...
        if not input_text.strip():
            ...
            with st.spinner("Generating..."):
                try:
                    ...
                    # Example for TTS: result = await client.generate_speech(
                    #     text=input_text, voice=voice
                    # )
                    # Example for images: result = await client.generate_image(
                    #     prompt=input_text, size="1024x1024"
                    # )
                    result = await client.generate_content(input_text)
